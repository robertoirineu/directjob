...
export default {
  ...
  methods: {
    salvarVaga() {
      ...
      let dataPublcacao = new Date(dataAtual)
      ...
      vagas.push({
        ...
        tipo: this.tipo,
        publicacao: dataPublcacao.toISOString()
      })
      
      if(this.validaFormulario()){
          localStorage.setItem('vagas', JSON.stringify(vagas));
          this.emitter.emit('alerta', {titulo:`A vaga ${this.titulo} foi cadastrada com sucesso`, mensagem:'Vaga já pode ser consultada por profissinais', tipoAlerta:'sucesso' });
          this.limparVaga();
      } else{
        this.emitter.emit('alerta', {titulo:`Vaga ${this.titulo} não cadastrada`, mensagem:'Algun(s) dado(s) não foi/foram preencidos', tipoAlerta:'erro' });
      }
